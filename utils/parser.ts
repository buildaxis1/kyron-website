 ...
 */
async function* parseCSV_better(
  input: File,
  header: Boolean,
): AsyncGenerator<Datapoint, null> {
  // Input is a file
  ...
  // Select the regex that will work for the special cases from the bottom of the file
  const regex = /,(?=(?:[^"]*"[^"]*")*[^"]*$)/;

  for (const line of lines) {
    // Modify to handle special cases that are tested in parser.test.ts
    ...
    yield cells.reduce(
      (prev: Datapoint, curr: string, idx: number) => {
        prev.length = idx + 1;
        // After splitting on the regex, see if you can figure out how to replace the escaped quotes and commas
        ...
        // For header-mapped values, apply the same unquoting and escape handling
